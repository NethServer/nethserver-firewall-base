<template>
  <div>
    ...
    <button
      ...
      class="btn btn-primary btn-lg"
    >{{$t('rules.create_priority_rule')}}</button>
          ...
          <form class="form-horizontal" v-on:submit.prevent="saveTc()">
            <div class="modal-body">
              ...
              <legend class="fields-section-header-pf" aria-expanded="true">
                <span
                  :class="['fa fa-angle-right field-section-toggle-pf', newTc.advanced ? 'fa-angle-down' : '']"
                ></span>
                <a
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <form class="form-horizontal" v-on:submit.prevent="deleteTc(currentTc)">
            <div class="modal-body">
              <div v-show="currentTc.isError" class="alert alert-warning alert-dismissable">
                <span class="pficon pficon-warning-triangle-o"></span>
                <strong>{{$t('warning')}}.</strong>
                {{$t('validation.'+currentTc.isError)}}.
              </div>
              <div class="form-group">
                <label
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h4
              class="modal-title"
            >{{newRule.isEdit ? $t('rules.edit_rule') : newRule.isDuplicate ? $t('rules.duplicate_rule') : $t('rules.create_priority_rule')}}</h4>
          </div>
...
export default {
  ...
  methods: {
    ...
    saveTc() {
      ...
      var tcObj = {
        action: context.newTc.isEdit ? "update-class" : "create-class",
      ...
      nethserver.exec(
        ...
        function(error, data) {
          ...
          try {
            ...
            context.currentTc.isError = errorData.attributes[0].error;
            context.$forceUpdate();
          } catch (e) {
            console.error(e);
          }
